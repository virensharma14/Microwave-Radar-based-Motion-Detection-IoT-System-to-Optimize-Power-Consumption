...
const char* mqtt_server = "172.22.64.212";// your MQTT broker IPv4 adress - THIS IS DIFFERENT FOR YOU, pls check yours with "ipconfig"  in cmd line
...
void callback(char* topic, byte* message, unsigned int length) {
  ...
  if (String(topic) == DEVICE_2_OUTPUT) {//Check if the MQTT message matches the subscription, if add more ESP32 change topic esp32 to for instance esp33
...
void reconnect() {
  while (!client.connected()) {   // Loop until connected to the MQTT server
    ...
    if (client.connect("ESP8266Client")) {     // Try to connect
      ...
      client.subscribe(DEVICE_2_OUTPUT);      // Subscribe to topic, if add more ESP32 change topic esp32 to for instance esp33
    } 
    else {
...
void loop() {
  ...
  if (SensValue == HIGH && Value == 1) {    
    ...
    client.publish(DEVICE_2_STRING, States[3]);
    if(currentmillis-previousmillis >= interval){
      ...
      if(sense==1){
        ...
        client.publish(DEVICE_2_STRING, States[0]);
        digitalWrite(Bulb, HIGH);
        ...
        SensValue=LOW;
        Serial.println("LOW2");
        client.publish(DEVICE_2_STRING, States[4]);
    ...
    if(Value==1){     
      ...
      if(SensValue){
        ...
        client.publish(DEVICE_2_STRING, States[1]);
      }
      else{
        Serial.println("LOW1");
        client.publish(DEVICE_2_STRING, States[2]);
      }
    }
    else{
      Serial.println("OFF");
      client.publish(DEVICE_2_STRING, States[5]);
      SensValue = 0;
      digitalWrite(Bulb,SensValue);
    }
  }
